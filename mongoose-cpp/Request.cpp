...
static int mg_get_cookie(const char *cookie_header, const char *var_name, char *dst, size_t dst_size) {
    const char *s, *p, *end;
    int name_len, len = -1;
...
namespace Mongoose {
Request::Request(struct mg_connection *connection_) : connection(connection_) {
...
void Request::handleUploads() {
    ...
    if (mg_parse_multipart(connection->content, connection->content_len, var_name, sizeof(var_name), file_name,
                           sizeof(file_name), &data, &data_len)) {
        uploadFiles.push_back(UploadFile(string(file_name), string(data, data_len)));
    }
}

map<string, string> Request::getHeaders() { return headers; }
};
